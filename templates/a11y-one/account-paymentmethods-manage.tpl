...
<div class="card">
    <div class="card-body">
        ...
        <form id="frmManagePaymentMethod" class="frm-credit-card-input" role="form" method="post" action="{if $editMode}{routePath('account-paymentmethods-save', $payMethod->id)}{else}{routePath('account-paymentmethods-add')}{/if}">
            <div class="alert alert-warning text-center gateway-errors assisted-cc-input-feedback w-hidden" role="alert">
                {lang key='paymentMethodsManage.invalidCardDetails'}
            </div>

            <fieldset class="form-group row">
                <legend id="inputPaymentMethodType" class="col-md-4 control-label col-form-label">{lang key='paymentMethods.type'}</legend>
                <div class="col-md-8">
                    ...
                    {if $creditCardCvcFieldEnabled}
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                ...
                                <button id="cvvWhereLink" type="button" class="btn btn-link" data-toggle="popover" data-content="<img src='{$BASE_PATH_IMG}/ccv.gif' width='210' alt='{lang key='creditcardcvvwhere'}'>" aria-label="{lang key='creditcardcvvwhere'}">
            ...
            <div class="fieldgroup-bankaccount{if $remoteUpdate || ($editMode && $payMethod->isCreditCard()) || ($paymentMethodType != 'bankacct' && !$editMode)} w-hidden{/if}">
                <fieldset class="form-group row">
                    <legend class="col-md-4 control-label col-form-label">{lang key='paymentMethodsManage.accountType'}</legend>
                    <div class="col-md-8">
                        <label class="form-check form-check-inline">
            ...
            <div class="fieldgroup-auxfields{if $remoteUpdate} w-hidden{/if}">
                <div class="form-group row">
                    ...
                    <div class="col-md-8">
                        ...
                        <br>
                        <button type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="#modalBillingAddress">{lang key='paymentMethodsManage.addNewAddress'}</button>
                    </div>
                </div>
        ...
        <div class="fieldgroup-remoteinput{if ($editMode && !$remoteUpdate) || !$editMode} w-hidden{/if}">
            ...
            {else}
                ...
                <div class="text-center">
                    <iframe name="ccframe" class="auth3d-area" width="90%" height="600" scrolling="auto" src="about:blank" title="{lang key='a11ySecurePaymentFrame'}"></iframe>
                </div>
            {/if}
        </div>
...
<div class="modal fade" id="modalBillingAddress" tabindex="-1" role="dialog" aria-labelledby="modalBillingAddressLabel">
        ...
        <div class="modal-content">
            <form id="billingContactForm" action="{routePath('account-paymentmethods-billing-contacts-create')}" data-role="json-form">
                ...
                <div class="modal-header">
                    <h2 class="modal-title h4" id="modalBillingAddressLabel">{lang key='paymentMethodsManage.addNewBillingAddress'}</h2>
...
<script>
    ...
    jQuery(document).ready(function() {
        ...
        jQuery('input[name="type"]').on('ifChecked change', function(e) {
        ...
        jQuery('input[name="billingcontact"]').on('ifChecked change', function(e) {
        ...
        if (jQuery('input[name="type"]:checked', ccForm).length === 0) {
            jQuery('input[name="type"]', ccForm).first().trigger('change');
        }
