...
<body>
    ...
    <main id="main-body" tabindex="-1" class="container-fluid invoice-container">
        ...
        {else}
            ...
            <div class="row invoice-header">
                ...
                <div class="col-12 col-sm-6 invoice-col text-center">
                    ...
                    <div class="invoice-status">
                        ...
                        {/if}
                        <span class="sr-only">: {lang key='invoicesstatus'}</span>
                    </div>
            ...
            <div class="row">
                <div class="col-12 col-sm-6 order-sm-last text-sm-right invoice-col right">
                    <strong><label for="inputGatewaySelect">{lang key='paymentmethod'}</label></strong><br>
                    <span class="small-text float-sm-right" data-role="paymethod-info">
                        {if $status eq "Unpaid" && $allowchangegateway}
                            <form method="post" action="{$smarty.server.PHP_SELF}?id={$invoiceid}" class="form-inline">
                                {$tokenInput}
                                <select name="gateway" id="inputGatewaySelect" class="custom-select" onchange="submit()">
            ...
            {if $manualapplycredit}
                <div class="card mb-3">
                    <div class="card-header bg-success text-white">
                        <h2 class="card-subtitle h5 mb-0"><strong>{lang key='invoiceaddcreditapply'}</strong></h2>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{$smarty.server.PHP_SELF}?id={$invoiceid}">
                            <input type="hidden" name="applycredit" value="true" />
                            {lang key='invoiceaddcreditdesc1'} <strong>{$totalcredit}</strong>. {lang key='invoiceaddcreditdesc2'}. <label for="inputCreditAmount">{lang key='invoiceaddcreditamount'}</label>:
                            <div class="row">
                                <div class="col-8 offset-2 col-sm-4 offset-sm-4">
                                    <div class="input-group">
                                        <input type="text" name="creditamount" id="inputCreditAmount" value="{$creditamount}" class="form-control" />
            ...
            <div class="card bg-default">
                <div class="card-header">
                    <h2 class="card-title mb-0 font-size-24 h4"><strong>{lang key='invoicelineitems'}</strong></h2>
                </div>
                <div class="table-responsive">
                    <table class="table table-sm">
                        <caption class="sr-only">{lang key='invoicelineitems'}</caption>
                        <thead>
                        <tr>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    {if $transaction.referenceHref}
                                        <a href="{$transaction.referenceHref}" target="_blank" rel="noopener">
                                    {/if}
                                    ...
                                    {if $transaction.referenceHref}
                                        <span class="sr-only">({lang key='a11yOpensInNewWindow'})</span></a>
                                    {/if}
                                </td>
            ...
            <div class="float-right btn-group btn-group-sm d-print-none">
                ...
                <a href="dl.php?type=i&amp;id={$invoiceid}" class="btn btn-default"><i class="fas fa-download" aria-hidden="true"></i> {lang key='invoicesdownload'}</a>
            </div>
