{*
    A11y One — accessible "open a ticket" step two (compose form).
    Author: Can Kirca <cankirca@gmail.com> — https://github.com/cankirca

    Accessibility changes vs parent twenty-one:
      - All selects (department / related service / priority) keep persistent
        visible <label for> (already present; preserved).
      - Markdown editor (bootstrap-markdown widget): toolbar buttons, the editor
        textarea and the status footer are enhanced AFTER the widget initialises
        by a11y-one.js (toolbar buttons get aria-label+title, the textarea gets an
        aria-label, the preview toggle gets aria-pressed, the status footer gets
        aria-live="polite"). The textarea carries data-a11y-mde-label so the JS
        can give the CodeMirror/textarea a localised accessible name without a
        hardcoded string.
      - File upload: the first input keeps a real <label for>. The accepted-types
        / max-size note is referenced from every file input via aria-describedby
        (#attachmentTypesHelp). The JS "add more" clone is fixed in a11y-one.js to
        mint a unique id + matching <label for> for each cloned input and to
        announce the new field via an aria-live region.
      - KB suggestions: the #autoAnswerSuggestions container is a named live
        region (role="region" + aria-label + aria-live="polite") so AJAX-injected
        suggestions are announced. The polling trigger (getTicketSuggestions) is
        moved out of an inline <script> into a11y-one.js via data-a11y-kb-suggest.
*}
...
<div class="card">
    <div class="card-body extra-padding">
        ...
        <div class="form-group">
            ...
            <textarea name="message" id="inputMessage" rows="12" class="form-control markdown-editor" data-auto-save-name="client_ticket_open" data-a11y-mde-label="{lang key='contactmessage'}">{$message}</textarea>
        </div>
        ...
        <div id="autoAnswerSuggestions" class="w-hidden" role="region" aria-label="{lang key='kbsuggestions'}" aria-live="polite"></div>
...
{if $kbsuggestions}
    {* a11y-one: KB-suggestion polling trigger moved out of an inline <script>.
       a11y-one.js starts getTicketSuggestions() when this marker is present. *}
    <span id="a11yKbSuggestTrigger" data-a11y-kb-suggest="1" hidden></span>
{/if}
