{* A11y One — clientareaproductdetails.tpl override.
   Author: Can Kirca
   WS-D Task 1: single <h1>, decorative icons aria-hidden, tabs from WS-A helper,
   copy-to-clipboard + password reveal via WS-A JS, hookOutput labelled region.
   DEFERRAL: module-injected markup in {$moduleclientarea} / {$tplOverviewTabOutput}
   cannot be controlled from this template; SSL state <img> from module is also
   beyond template control. Both are tracked as known deferrals. *}
...
<div class="tab-content margin-bottom">
    <div class="tab-pane fade show active" role="tabpanel" id="tabOverview">
        ...
        <div class="card">
                                    ...
                                    {if $packagesupgrade}
                                        <div class="col-12">
                                            <a href="upgrade.php?type=package&amp;id={$id}" class="btn btn-block btn-success">
                                                <i class="fas fa-level-up" aria-hidden="true"></i>
                                                {lang key='upgrade'}
                                            </a>
                                        </div>
                                    {/if}
            ...
            {if $hookOutput}
                {* Wrap hook-injected output in a labelled region so AT can identify it *}
                <section aria-label="{lang key='productDetails.hookOutputLabel'}">
            ...
            {if $domain || $moduleclientarea || $configurableoptions || $customfields || $lastupdate}
                ...
                <ul class="nav nav-tabs responsive-tabs-sm">
                    {if $domain}
                        <li class="nav-item">
                            <a href="#domain" data-toggle="tab" class="nav-link active">
                                ...
                                {if $type eq "server"}{lang key='sslserverinfo'}{elseif ($type eq "hostingaccount" || $type eq "reselleraccount") && $serverdata}{lang key='hostingInfo'}{else}{lang key='clientareahostingdomain'}{/if}
                            </a>
                        </li>
                    {elseif $moduleclientarea}
                        <li class="nav-item">
                            <a href="#manage" data-toggle="tab" class="nav-link active">
                                <i class="fas fa-globe fa-fw" aria-hidden="true"></i>
                                {lang key='manage'}
                            </a>
                        </li>
                    ...
                    {if $configurableoptions}
                        <li class="nav-item">
                            <a href="#configoptions" data-toggle="tab" class="nav-link{if !$domain && !$moduleclientarea} active{/if}">
                                <i class="fas fa-cubes fa-fw" aria-hidden="true"></i>
                                {lang key='orderconfigpackage'}
                            </a>
                        </li>
                    {/if}
                    {if $metricStats}
                        <li class="nav-item">
                            <a href="#metrics" data-toggle="tab" class="nav-link{if !$domain && !$moduleclientarea && !$configurableoptions} active{/if}">
                                <i class="fas fa-chart-line fa-fw" aria-hidden="true"></i>
                                {lang key='metrics.title'}
                            </a>
                        </li>
                    {/if}
                    {if $customfields}
                        <li class="nav-item">
                            <a href="#additionalinfo" data-toggle="tab" class="nav-link{if !$domain && !$moduleclientarea && !$metricStats && !$configurableoptions} active{/if}">
                                <i class="fas fa-info fa-fw" aria-hidden="true"></i>
                                {lang key='additionalInfo'}
                            </a>
                        </li>
                    {/if}
                    {if $lastupdate}
                        <li class="nav-item">
                            <a href="#resourceusage" data-toggle="tab" class="nav-link{if !$domain && !$moduleclientarea && !$configurableoptions && !$customfields} active{/if}">
                                <i class="fas fa-inbox fa-fw" aria-hidden="true"></i>
                                {lang key='resourceUsage'}
                            </a>
                        </li>
                    {/if}
                </ul>
                    ...
                    {if $domain}
                        <div class="tab-pane fade show active text-center" role="tabpanel" id="domain">
                            ...
                            {else}
                                ...
                                {if $domain && $sslStatus}
                                    <div class="row">
                                        ...
                                        <div class="col-sm-7 text-left{if $sslStatus->isInactive()} ssl-inactive{/if}">
                                            {* Module-injected SSL image — alt text from module data *}
                                            <img src="{$sslStatus->getImagePath()}" alt="" width="12" data-type="service" data-domain="{$domain}" data-showlabel="1" class="{$sslStatus->getClass()}"/>
                                ...
                                <br>
                                <p>
                                    <a href="http://{$domain}" class="btn btn-default" target="_blank" rel="noopener noreferrer">{lang key='visitwebsite'}</a>
                                    {if $domainId}
                                        <a href="clientarea.php?action=domaindetails&id={$domainId}" class="btn btn-default" target="_blank" rel="noopener noreferrer">{lang key='managedomain'}</a>
                                    {/if}
                                </p>
                            {/if}
                            {if $moduleclientarea}
                        ...
                        <div class="tab-pane fade text-center" role="tabpanel" id="resourceusage">
                            <div class="row">
                                <div class="col-sm-10 offset-sm-1">
                                    <div class="row">
                                        <div class="col-sm-6">
                                            <h2 class="h4">{lang key='diskSpace'}</h2>
                                        ...
                                        <div class="col-sm-6">
                                            <h2 class="h4">{lang key='bandwidth'}</h2>
    ...
    <div class="tab-pane fade" role="tabpanel" id="tabDownloads">
        ...
        <h2 class="h3">{lang key='downloadstitle'}</h2>
        ...
        <div class="row">
            {foreach from=$downloads item=download}
                <div class="col-10 offset-1">
                    ...
                    <p>
                        <a href="{$download.link}" class="btn btn-default"><i class="fas fa-download" aria-hidden="true"></i> {lang key='downloadname'}</a>
                    </p>
                </div>
            {/foreach}
        </div>
    ...
    <div class="tab-pane fade" id="tabAddons">
        ...
        <h2 class="h3">{lang key='clientareahostingaddons'}</h2>
    ...
    <div class="tab-pane fade" id="tabChangepw">
        ...
        <div class="card">
            <div class="card-body">
                <h2 class="h3 card-title">{lang key='serverchangepassword'}</h2>
                ...
                <form class="using-password-strength" method="post" action="{$smarty.server.PHP_SELF}?action=productdetails#tabChangepw" role="form">
                    ...
                    <div id="newPassword1" class="form-group row has-feedback">
                        ...
                        <div class="col-xl-5">
                            <input type="password" class="form-control" id="inputNewPassword1" name="newpw" autocomplete="new-password" />
                    ...
                    <div id="newPassword2" class="form-group row has-feedback">
                        ...
                        <div class="col-xl-5">
                            <input type="password" class="form-control" id="inputNewPassword2" name="confirmpw" autocomplete="new-password" />
